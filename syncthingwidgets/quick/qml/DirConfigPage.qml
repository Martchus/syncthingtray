...
AdvancedDirConfigPage {
    ...
    specialEntries: [
        ...
        {key: "paused", label: qsTr("Paused"), statusText: Qt.binding(() => !dirConfigPage.configObjectExists ? qsTr("Keep initially disabled to add ignore patterns before any syncing takes place.") : null), desc: qsTr("Whether this folder is (temporarily) suspended.")},
        ...
        {key: "blockIndexing", label: qsTr("Block Indexing"), desc: qsTr("Maintain an index of all blocks in the folder, enabling reuse of blocks from other files when syncing changes. Disable to reduce database size at the cost of not being able to reuse blocks across files.")},
